...
# Fits fixed effects meta-regression since most of the RE meta-regressions don't
# detect significant heterogeneity
################################################################################
...
for(i in 1:5) {
  ...
  fit <- rma.uni(mest,sampvar,mods=~meancsf,method="FE") # need to use method=SJ
...
for(i in 1:5) {
  ...
  fit <- rma.uni(mest,sampvar,mods=~meancsf,method="FE")
  print(fit)
  newx <- seq(min(meancsf),max(meancsf),len=10)
  preds <- predict(fit,newmods=newx)
